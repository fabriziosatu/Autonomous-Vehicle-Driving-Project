...
class BehaviorAgent(BasicAgent):
    ...
    def run_step(self, debug=False):
        ...
        if tw_state is True:
            ...
            if not (self._overtake_manager.in_overtake and tw_distance < 6) or (self._is_raining and tw_distance < 8):
    ...
    def collision_and_car_avoid_manager(self, waypoint : carla.Waypoint) -> tuple:
        ...
        vehicle_list = [v for v in vehicle_list if dist(v, waypoint) < 17 and v.id != self._vehicle.id]       
    ...
    def __is_approaching_curve(self, threshold_angle_deg=5.0, distance_ahead=2.0):
        """
        Check if the vehicle is approaching a curve by comparing the direction vectors of future waypoints.

        :param threshold_angle_deg: Minimum angle in degrees to consider as a curve.
        :param distance_ahead: Distance in meters to check ahead on the route.
        :return: True if a curve is detected, else False.
        """
        # Get current waypoint
        current_location = self._vehicle.get_location()
        current_wp = self._map.get_waypoint(current_location, project_to_road=True, lane_type=carla.LaneType.Driving)
        
        if current_wp is None:
            return False

        # Get next two waypoints
        next_wp1_list = current_wp.next(distance_ahead)
        if not next_wp1_list:
            return False
        next_wp1 = next_wp1_list[0]

        next_wp2_list = next_wp1.next(distance_ahead)
        if not next_wp2_list:
            return False
        next_wp2 = next_wp2_list[0]

        # Get forward vectors
        v1 = next_wp1.transform.get_forward_vector()
        v2 = next_wp2.transform.get_forward_vector()
        ...
        dot = (v1.x * v2.x + v1.y * v2.y) / (mag1 * mag2)
        dot = max(min(dot, 1.0), -1.0)  # Clamp dot product to valid acos range
    ...
    def __update_information(self, debug : bool = False) -> None:
        ...
        # elif isinstance(self._behavior, Normal):
        #     self._behavior = Normal()

        # elif isinstance(self._behavior, Aggressive):
        #     self._behavior = Aggressive()
            
        # if self._is_raining:
        #     self._behavior.max_speed -= 5
        #     self._behavior.braking_distance += 3
        #     self._behavior.min_proximity_threshold += 3
        ...
        # if self.__is_approaching_curve():
        #     self._behavior.max_speed -= 8
